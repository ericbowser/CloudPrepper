...
const EditQuestionForm: React.FC<EditQuestionFormProps> = ({
    ...
    useEffect(() => {
        if (question) {
            ...
            console.log('🔍 Loading question for editing:', question.question_id);
            ...
            // Parse explanation details
            let parsedExplanationDetails = question.explanation_details;
            if (typeof question.explanation_details === 'string') {
                try {
                    parsedExplanationDetails = JSON.parse(question.explanation_details);
                } catch (error) {
                    console.error('Error parsing explanation details:', error);
            ...
            // Ensure correctAnswersArray is an array
            ...
            // Find matching domain ID from the certification config
            const cert = CERTIFICATIONS.find(c => c.id === certification);
            const domainId = cert?.domains.find(d =>
                d.name.toLowerCase() === question.domain?.toLowerCase()
            )?.id || '';

            // Re-initialize formData from question prop
            const mappedOptions = Array.isArray(parsedOptions) && parsedOptions.length > 0
                ? parsedOptions.map(opt => {
                    const optionText = typeof opt === 'string' ? opt : opt.text || '';
                    const isCorrect = correctAnswersArray.includes(optionText) || 
                                     (opt as any).isCorrect === true;
                    console.log(`  Option "${optionText}" -> isCorrect: ${isCorrect}`);
                    return {
                        text: optionText,
                        isCorrect: isCorrect
                    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await onSubmit(question.question_id, updateData, formData.certification);
            setHasChanges(false);
        } catch (error) {
        ...
        <div className="max-w-4xl mx-auto p-6 bg-pastel-mintlight dark:bg-dark-800 rounded-lg shadow-lg">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        {formData.certification === 'comptia' ? 'CompTIA Cloud+' : 'AWS Solutions Architect'} • {formData.category}
                    </p>
                </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <FormActions
                    ...
                    submitLabel={hasChanges ? 'Save Changes' : 'No Changes'}
                    isLoading={isLoading}
