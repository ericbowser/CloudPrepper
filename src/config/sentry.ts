...
export function initSentry() {
  ...
  const environment = VITE_SENTRY_ENVIRONMENT || 'development';
...
// Export standalone API helper functions for use throughout the app
export function apiRequest(endpoint: string, method: string) {
  addBreadcrumb('api', `API request: ${method} ${endpoint}`, { endpoint, method });
}

export function apiSuccess(endpoint: string, status: number | string) {
  addBreadcrumb('api', `API success: ${endpoint}`, { endpoint, status });
}

export function apiError(endpoint: string, error: string) {
  addBreadcrumb('api', `API error: ${endpoint}`, { endpoint, error }, 'error');
}
